...
# tries to open preferences and set schoolName and menuSelection variables
try:
...
def determineWhichMenuToGrab(argv):
    ...
    elif argv != 0 and argv[:1] == "+" and isInt(argv[1:]):
        datezers += theCENTER.timedelta(int(argv))
    if datezers.weekday() == 6 or datezers.weekday() == 5:
